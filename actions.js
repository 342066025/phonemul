...
export async function commitStagedActions() {
    ...
    playerActionsToCommit.forEach(action => {
        ...
        switch(action.type) {
            ...
            case 'new_forum_reply': {
                ...
                if (post) {
                    textPrompt += `- 在论坛帖子"${post.title}"下发表了回复，内容为"${action.content}"\\n`;
                } else {
                    textPrompt += `- 在论坛发表了回复（帖子ID: ${action.postId}），内容为"${action.content}"\\n`;
                }
                break;
            }
        }
    ...
    if (hasActionsForAI) {
        try {
            await SillyTavern_Main_API.triggerSlash(`/setinput ${JSON.stringify(textPrompt)}`);
            SillyTavern_Context_API.generate();
        } catch (error) {
