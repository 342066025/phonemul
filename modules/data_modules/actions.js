...
// 获取当前角色名，如果没有选择角色则返回{{user}}
function getCurrentCharacterName() {
    return PhoneSim_State.currentCharacter || '{{user}}';
}

export async function getOrCreatePhoneLorebook(targetCharacterName = null) {
    // 使用指定的角色名称，如果没有指定则使用当前选择的角色或SillyTavern的当前角色
    let characterName = targetCharacterName;
    if (!characterName) {
        // 优先使用手机模拟器当前选择的角色
        if (PhoneSim_State.currentCharacter) {
            characterName = PhoneSim_State.currentCharacter;
        } else {
            // 回退到SillyTavern的当前角色
            characterName = SillyTavern_Main_API.getContext().name2;
        }
    }
    
    console.log(`[Phone Sim] getOrCreatePhoneLorebook: 目标角色=${characterName}, 缓存角色=${currentCharacterName}`);
    ...
    if (currentCharacterName === characterName && currentCharacterLorebookName) {
        console.log(`[Phone Sim] getOrCreatePhoneLorebook: 使用缓存的世界书=${currentCharacterLorebookName}`);
        return currentCharacterLorebookName;
    }

    currentCharacterName = characterName;
    const lorebookName = `${PhoneSim_Config.LOREBOOK_PREFIX}${characterName}`;
    console.log(`[Phone Sim] getOrCreatePhoneLorebook: 生成世界书名称=${lorebookName}`);
    ...
    if (!allLorebooks.includes(lorebookName)) {
        console.log(`[Phone Sim] getOrCreatePhoneLorebook: 创建新世界书=${lorebookName}`);
        await TavernHelper_API.createOrReplaceWorldbook(lorebookName, PhoneSim_Config.INITIAL_LOREBOOK_ENTRIES);
    } else {
        console.log(`[Phone Sim] getOrCreatePhoneLorebook: 世界书已存在=${lorebookName}`);
    }
    ...
    if (charLorebooks && !charLorebooks.additional.includes(lorebookName)) {
        console.log(`[Phone Sim] getOrCreatePhoneLorebook: 绑定世界书到当前角色=${lorebookName}`);
        ...
        console.log(`[Phone Sim] getOrCreatePhoneLorebook: 世界书已绑定=${lorebookName}`);
    }
    
    // Cache and return the name
    currentCharacterLorebookName = lorebookName;
    console.log(`[Phone Sim] getOrCreatePhoneLorebook: 缓存并返回世界书名称=${lorebookName}`);
    return lorebookName;
}


export async function _updateWorldbook(dbName, updaterFn) {
    // 确保使用当前选择的角色，而不是缓存的角色
    const currentChar = PhoneSim_State.currentCharacter;
    console.log(`[Phone Sim] _updateWorldbook: 当前角色=${currentChar}, 数据库=${dbName}`);
    ...
    console.log(`[Phone Sim] _updateWorldbook: 使用世界书=${lorebookName}`);
...
/**
 * 跨角色数据写入函数 - 向指定角色的世界书写入数据
 * @param {string} targetCharacterName - 目标角色名称
 * @param {string} dbName - 数据库名称
 * @param {function} updaterFn - 数据更新函数
 * @returns {boolean} - 操作是否成功
 */
export async function _updateCrossCharacterWorldbook(targetCharacterName, dbName, updaterFn) {
    if (!targetCharacterName) {
        console.error("[Phone Sim] _updateCrossCharacterWorldbook: 目标角色名称不能为空");
        return false;
    }
    
    console.log(`[Phone Sim] _updateCrossCharacterWorldbook: 向角色 ${targetCharacterName} 的数据库 ${dbName} 写入数据`);
    
    try {
        // 获取目标角色的世界书名称
        const targetLorebookName = await getOrCreatePhoneLorebook(targetCharacterName);
        if (!targetLorebookName) {
            console.error(`[Phone Sim] 无法获取或创建角色 ${targetCharacterName} 的世界书`);
            return false;
        }
        
        console.log(`[Phone Sim] _updateCrossCharacterWorldbook: 使用目标世界书=${targetLorebookName}`);

        let entries = await TavernHelper_API.getWorldbook(targetLorebookName);
        if (!Array.isArray(entries)) {
            entries = [];
        }
        ...
        if (dbIndex === -1) {
            console.log(`[Phone Sim] 在目标角色世界书中创建新的数据库条目: ${dbName}`);
            const newEntry = { 
                name: dbName, 
                content: defaultContent, 
                enabled: false,
        ...
        try {
            ...
            console.warn(`[Phone Sim] 无法解析目标角色的 ${dbName} 内容，重置为默认值. 内容:`, entries[dbIndex].content, e);
            dbData = JSON.parse(defaultContent);
        }
        ...
        await TavernHelper_API.replaceWorldbook(targetLorebookName, entries);
        console.log(`[Phone Sim] 成功向角色 ${targetCharacterName} 的数据库 ${dbName} 写入数据`);
        return true;

    } catch (er) {
        console.error(`[Phone Sim] 跨角色数据写入错误 (目标角色: ${targetCharacterName}, 数据库: ${dbName}):`, er);
        return false;
    }
}

/**
 * 获取角色映射关系 - 联系人ID到角色名称的映射
 * @returns {Object} - 角色映射对象 {contactId: characterName}
 */
export async function getCharacterMapping() {
    try {
        const lorebookName = await getOrCreatePhoneLorebook();
        if (!lorebookName) {
            console.error("[Phone Sim] 无法获取世界书来读取角色映射");
            return {};
        }

        const entries = await TavernHelper_API.getWorldbook(lorebookName);
        const mappingEntry = entries.find(e => e.name === PhoneSim_Config.WORLD_CHARACTER_MAPPING);
        
        if (mappingEntry) {
            return JSON.parse(mappingEntry.content || '{}');
        }
        return {};
    } catch (error) {
        console.error("[Phone Sim] 获取角色映射失败:", error);
        return {};
    }
}
 ...
 */
export async function updateCharacterMapping(contactId, characterName) {
    if (!contactId || !characterName) {
        console.error("[Phone Sim] updateCharacterMapping: contactId和characterName不能为空");
        return false;
    }

    console.log(`[Phone Sim] 更新角色映射: ${contactId} -> ${characterName}`);
    
    return await _updateWorldbook(PhoneSim_Config.WORLD_CHARACTER_MAPPING, mappingData => {
        mappingData[contactId] = characterName;
        return mappingData;
    });
}
 ...
 * 根据联系人ID获取对应的角色名称
 * @param {string} contactId - 联系人ID
 * @returns {string|null} - 角色名称，如果未找到返回null
 */
export async function getCharacterNameByContactId(contactId) {
    const mapping = await getCharacterMapping();
    return mapping[contactId] || null;
}
...
export async function initiateVoiceCall(contactId) {
    ...
    const prompt = `(系统提示：${getCurrentCharacterName()}向${contactName}发起了微信语音通话...)`;
...
export async function initiatePhoneCall(callTarget) {
    const prompt = `(系统提示：${getCurrentCharacterName()}正在呼叫${callTarget.name}的电话...)`;
    await TavernHelper_API.triggerSlash(`/setinput ${JSON.stringify(prompt)}`);
    SillyTavern_Context_API.generate();
    UI.closeCallUI();
...
export async function commitStagedActions() {
    ...
    let textPrompt = `(系统提示：${getCurrentCharacterName()}刚刚在手机上进行了如下操作：\\n`;
        ...
        finalMessagesToPersist.forEach(item => {
            ...
            if (contactObject?.app_data?.WeChat) {
                ...
                const enhancedMessage = {
                    ...item.message,
                    sender_character: PhoneSim_State.currentCharacter,
                    receiver_id: item.contactId,
                    isCrossCharacter: true
                };
                
                contactObject.app_data.WeChat.messages.push(enhancedMessage);
                
                // 收集需要跨角色同步的消息
                crossCharacterMessages.push({
                    contactId: item.contactId,
                    message: enhancedMessage,
                    senderCharacter: PhoneSim_State.currentCharacter
                });
            }
        });
    ...
    // 处理跨角色消息同步
    if (crossCharacterMessages.length > 0) {
        try {
            for (const crossMessage of crossCharacterMessages) {
                // 获取接收者角色名称
                const receiverCharacterName = await getCharacterNameByContactId(crossMessage.contactId);
                
                if (receiverCharacterName && receiverCharacterName !== crossMessage.senderCharacter) {
                    // 更新角色映射
                    await updateCharacterMapping(crossMessage.contactId, receiverCharacterName);
                    
                    // 将消息同步到接收者的世界书
                    await _updateCrossCharacterWorldbook(receiverCharacterName, PhoneSim_Config.WORLD_DB_NAME, dbData => {
                        // 在接收者的世界书中找到发送者的联系人记录
                        const senderContactId = Object.keys(dbData).find(id => {
                            const contact = dbData[id];
                            return contact?.character_name === crossMessage.senderCharacter;
                        });
                        ...
                        if (senderContactId && dbData[senderContactId]?.app_data?.WeChat) {
                            ...
                            const receiverMessage = {
                                ...
                                sender_id: senderContactId, // 在接收者视角中，发送者ID是联系人ID
            ...
            console.error('[Phone Sim] Failed to sync cross-character messages:', error);
        }
    }
