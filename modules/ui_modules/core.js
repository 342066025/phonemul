...
function _createToggleButton() {
    const buttonHtml = `
        <div id="${PhoneSim_Config.TOGGLE_BUTTON_ID}" title="手机模拟器多角色版">
...
export async function initializeUI() {
    try {
        ...
        console.info(`[Phone Sim] Fetching panel from: ${panelUrl}`);
        ...
        if (parentWin.toastr) {
            parentWin.toastr.error("手机模拟器多角色版插件UI加载失败，请检查控制台以获取详细信息并尝试刷新页面。", "严重错误", { timeOut: 10000 });
        }
        return false;
    }
}
