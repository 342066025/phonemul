...
const loggingPrefix = '[手机模拟器多角色版 v17.0]';
...
function onSettingChanged() {
    ...
    if (parentWin.toastr) {
        parentWin.toastr.info('设置已保存。刷新页面以应用更改。', '手机模拟器多角色版');
    }
}
            ...
            <div class="inline-drawer-content">
                <div class="block">
                    <label class="flex-container">
                        ...
                        <span>启用手机模拟器多角色版</span>
                    </label>
                </div>
                <small>禁用后，手机模拟器多角色版将不会加载悬浮窗或处理任何指令。更改后需要刷新页面才能完全生效。</small>
            </div>
        </div>
    </div>`;
...
async function mainInitialize() {
    ...
    PhoneSim_State.loadCurrentCharacter();
    PhoneSim_State.loadAvailableCharacters();
    
    // Set current character if not already set
    if (!PhoneSim_State.currentCharacter && SillyTavern_Context && SillyTavern_Context.name2) {
        PhoneSim_State.currentCharacter = SillyTavern_Context.name2;
        PhoneSim_State.saveCurrentCharacter();
        PhoneSim_State.addCharacterToList(SillyTavern_Context.name2);
    }
    ...
    // 将DataHandler暴露到window对象，供角色切换时使用
    window.PhoneSim_DataHandler = PhoneSim_DataHandler;
    window.PhoneSim_UI = PhoneSim_UI;
    
    // Load test script for data isolation testing (development only)
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        try {
            import('./test_data_isolation.js').then(() => {
                console.log(`%c${loggingPrefix} 数据隔离测试脚本已加载`, 'color: #2196f3;');
            }).catch(err => {
                console.log(`%c${loggingPrefix} 测试脚本加载失败（这在生产环境中是正常的）`, 'color: #ff9800;');
            });
        } catch (err) {
